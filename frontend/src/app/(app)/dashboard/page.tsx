...
function StatCard({ label, value, unit, icon, status, trend, description }: {
  label: string; value: string; unit: string
  icon: React.ReactNode; status: "ok" | "warning" | "critical"
  trend?: "up" | "down" | "stable"; description?: string
}) {
  const statusColors = {
    ok:       { bg: "#f0faf0", border: "#c8e8c8", badge: "#16a34a" },
    warning:  { bg: "#fffbeb", border: "#fde68a", badge: "#d97706" },
    critical: { bg: "#fff1f1", border: "#fecaca", badge: "#dc2626" },
  }
  ...
  return (
    <div
      className="rounded-2xl p-5 flex flex-col gap-4 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md"
    ...
    >
      ...
      <div>
        <div className="flex items-baseline gap-1.5 mb-1">
          ...
          {trend && <TrendIcon className="w-4 h-4 ml-1" style={{ color: trendColor }} />}
        </div>
...
function SectionCard({ title, subtitle, icon, action, children }: {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid #e8f0e8", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
      <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${LIME}20` }}>
  ...
  return (
    <>
      ...
      <div className="mx-6 mt-6 rounded-2xl overflow-hidden relative"
        ...
        <div className="relative z-10 px-8 py-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight leading-tight mb-2">{farm.name}</h1>
            <div className="flex flex-wrap items-center gap-4">
              <span className="flex items-center gap-1.5 text-sm" style={{ color: "rgba(255,255,255,0.6)" }}>
                <MapPin className="w-3.5 h-3.5" />{location}
              </span>
              {farm.area_hectares && (
                <span className="text-sm" style={{ color: "rgba(255,255,255,0.6)" }}>{farm.area_hectares} ha</span>
              )}
              {kpis && (
                <span className="flex items-center gap-1.5 text-sm" style={{ color: "rgba(255,255,255,0.6)" }}>
                  <Activity className="w-3.5 h-3.5" />{kpis.readings_count_24h} lecturas hoy
                </span>
              )}
            </div>
          </div>
          {hasSensorData && (
            <div className="shrink-0 text-center px-6 py-4 rounded-xl"
              ...
              <p className="text-[11px] font-bold uppercase tracking-widest mb-1" style={{ color: "rgba(255,255,255,0.5)" }}>Salud del suelo</p>
              ...
              <div className="mt-2 w-20 h-1.5 bg-white/10 rounded-full overflow-hidden mx-auto">
                <div className="h-full rounded-full transition-all duration-700" style={{ width: `${kpis!.soil_health_score}%`, background: LIME }} />
              </div>
            </div>
          )}
        </div>
      ...
      <div className="px-6 pb-8 space-y-5 mt-5">
        ...
        {!hasSensorData && (
          <div className="rounded-2xl p-5 flex flex-col sm:flex-row items-center gap-4 border border-dashed"
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <p className="text-xs text-amber-700 font-medium">El nodo ESP32 aún no ha enviado datos. Conecta el nodo a WiFi o configura el MQTT.</p>
            </div>
            <Link href="/sensors"
              className="shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold transition-colors"
              style={{ background: "#fef3c7", color: "#92400e", border: "1px solid #fde68a" }}>
              <Activity className="w-3.5 h-3.5" />Ver sensores
            </Link>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="VPD" value={kpis?.vpd_kpa?.toFixed(2) ?? "—"} unit="kPa"
            status={kpis ? kpiStatus(kpis.vpd_kpa, 0.4, 1.6) : "ok"}
            icon={<Wind className="w-4 h-4" />} trend="stable" description="Déficit de presión de vapor" />
          <StatCard label="Humedad suelo" value={kpis?.avg_soil_moisture_pct?.toFixed(1) ?? "—"} unit="%"
            status={kpis ? kpiStatus(kpis.avg_soil_moisture_pct, 30, 80) : "ok"}
            icon={<Droplets className="w-4 h-4" />} trend="stable" />
          <StatCard label="Temperatura aire" value={kpis?.avg_air_temp_c?.toFixed(1) ?? "—"} unit="°C"
            status={kpis ? kpiStatus(kpis.avg_air_temp_c, 15, 35) : "ok"}
            icon={<Thermometer className="w-4 h-4" />} trend="stable" />
          <StatCard label="Riesgo plagas" value={kpis?.pest_risk?.risk_pct?.toString() ?? "—"} unit="%"
            status={kpis ? kpiStatus(kpis.pest_risk?.risk_pct ?? 0, 0, 40) : "ok"}
            icon={<Waves className="w-4 h-4" />} trend="stable" />
        </div>

        {/* Chart + Feed */}
        <div className="grid lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2">
            <SectionCard title="Humedad del Suelo" subtitle="Últimas 24 horas" icon={<BarChart3 className="w-4 h-4" />}
              action={hasTimeseries ? (
          ...
          <div>
            <SectionCard title="Live Feed" subtitle="Nodo ESP32" icon={<Wifi className="w-4 h-4" />}>
              <div className="p-4"><LiveFeed farmId={farm.id} /></div>
            </SectionCard>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-5">
          <div className="lg:col-span-3">
            <SectionCard title="Análisis con IA"
              ...
              <div className="p-5">
                {lastAnalysis ? <AnalysisResult analysis={lastAnalysis} /> : (
                  <div className="flex flex-col items-center justify-center py-10 gap-4 text-center">
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ background: "#f3e8ff", border: "1px solid #e9d5ff" }}>
                      <Brain className="w-7 h-7 text-purple-500" />
                    </div>
                    <div>
                      <p className="font-bold text-gray-800 mb-1">Sin análisis todavía</p>
                      <p className="text-sm text-gray-500 font-medium max-w-xs">Genera tu primer análisis agronómico con IA usando el clima de 7 días + sensores.</p>
                    </div>
                    <Link href="/analyze"
                      ...
                      <Brain className="w-4 h-4" />Analizar mi finca<ArrowRight className="w-3.5 h-3.5" />
                    </Link>
                  </div>
                )}
              </div>
          ...
          <div className="lg:col-span-2">
            <SectionCard title="Alertas activas"
              ...
              action={activeAlerts.length > 0 ? (
                <Link href="/alerts" className="flex items-center gap-1 text-xs font-semibold text-gray-500 hover:text-gray-900 transition-colors">
              ...
              <div className="p-4 space-y-2">
                {activeAlerts.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-10 gap-3 text-center">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `${LIME}15`, border: `1px solid ${LIME}30` }}>
